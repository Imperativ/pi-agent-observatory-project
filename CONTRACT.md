...
- Values: lists of strings for tools, skills, readAreas, writeAreas, restrictions, missingCredentials, runtimes; nonnegative finite numbers for uptimeSeconds, contextWindow, contextUsed, inputTokens, outputTokens, cost; rateLimits is a string or structured quota object ({fiveHour?: {used, total, remainingPercent, resetsAt}, weekly?: {used, total, remainingPercent, resetsAt}, detail?}); strings otherwise. Progress is `{completed: nonnegative number, total: positive number, basis: nonempty string}`, completed <= total; UI percentage only from this basis. Cost's unit is documented in its source (no assumed currency).
...
`npm start` → `node server.mjs`, Node >=22, no mandatory install/build. Default http://127.0.0.1:4318. Default bind is loopback; the single-owner LAN requirement additionally allows explicit `--host <private IPv4>` (RFC1918, never wildcard/public), without adding authentication or Internet exposure. Host/Origin checks must match the selected bind address; LAN access is not authenticated and needs a trusted network and local firewall boundaries. Read-only explicit route allowlist, reject traversal, foreign Host/Origin/cross-site fetch, no CORS, no arbitrary workspace files. CSP same-origin only, no remote sources; no control/write endpoint. Optional `--port 0` used by smoke tests. Export `createServer({statusPath?, configPath?})` for tests; start only when direct CLI.
...
`npm run status:init` creates ignored `agent-status.json` exclusively from sample (never overwrites). Any agent writes a complete snapshot to a unique temp file in the same folder, validates it, renames atomically; README includes precise instructions. The optional Pi extension holds `agent-status.lock` throughout its session, emits allowlisted lifecycle measurements/heartbeats, and marks shutdown; manual exporter cannot write concurrently. It may publish validated Pi session/agent start times, a bounded list of fixed lifecycle labels, numeric input/output usage summed from the active session branch, known active shell/file tools, and local Node/Windows-build runtime metadata. It never publishes prompt content, raw model IDs, session IDs or credential material; paths and local Git repository/branch are only published with the explicit `PI_DASHBOARD_INCLUDE_WORKSPACE=1` opt-in, never a remote URL. No inference about rights, progress, account-wide quotas or costs is allowed. Manually entered quota measurements retain their own observation time across heartbeats; editing one does not refresh the root Pi snapshot. No status update API. Missing usage stays unavailable.
